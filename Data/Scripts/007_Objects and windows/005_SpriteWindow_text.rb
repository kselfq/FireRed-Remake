...
class Window_AdvancedTextPokemon < SpriteWindow_Base
  ...
  # Override horizontal padding (left/right margin)
  def text_padding_x
    # Use 16 pixels if your graphic has a 16px wide border on the left/right.
    # Adjust this number to shift the text horizontally.
    return 16 
  end
  
  # Override vertical padding (top/bottom margin)
  def text_padding_y
    # This value is crucial for vertical centering in your 94px box.
    # The default text line height is ~32px.
    # 94 (box height) - 32 (text height) = 62 pixels remaining
    # 62 / 2 = 31 pixels (for perfect vertical centering)
    # Adjust this number to shift the text vertically.
    return 31 
  end
  
  # Ensure the contents rectangle uses your new custom padding
  def contents_x
    # This combines the RPG Maker window's inherent padding (usually 16 or 0) 
    # with your custom text padding.
    return self.padding + self.text_padding_x
  end
  
  def contents_y
    return self.padding + self.text_padding_y
  end
...
class Window_DrawableCommand < SpriteWindow_SelectableEx
  ...
  def drawCursor(index, rect)
    if self.index == index
      pbCopyBitmap(self.contents, @selarrow.bitmap, rect.x, rect.y + 6)   # TEXT OFFSET (counters the offset above)
    end
    return Rect.new(rect.x + 24, rect.y, rect.width - 16, rect.height)
  end
