...
class MoveSelectionSprite < Sprite
  ...
  def refresh
    w = @movesel.width
    h = @movesel.height / 2
    # ADAPTATION: Anchor to right
    self.x = 196 + (Graphics.width - 512)
    self.y = 97 + (self.index * 64)
...
class RibbonSelectionSprite < MoveSelectionSprite
  ...
  def refresh
    ...
    h = @movesel.height / 2
    # ADAPTATION: Anchor to right
    self.x = 205 + ((self.index % 4) * 68) + (Graphics.width - 512)
    self.y = 92 + ((self.index / 4).floor * 68)
    self.bitmap = @movesel.bitmap
    if self.preselected
      self.src_rect.set(0, h, w, h)
    else
...
class PokemonSummary_Scene
  ...
  RED_TEXT_SHADOW   = Color.new(192, 32, 40, 0)
  BLACK_TEXT_BASE   = Color.new(64, 64, 64)
  BLACK_TEXT_SHADOW = Color.new(192, 32, 40, 0)
  ...
  def pbStartScene(party, partyindex, inbattle = false)
    ...
    @sprites["itemicon"] = ItemIconSprite.new(30, 340, @pokemon.item_id, @viewport)
    ...
    # ADAPTATION: Anchor to right
    @sprites["uparrow"].x = 350 + (Graphics.width - 512) - 12
    @sprites["uparrow"].y = 56 + 16
    @sprites["uparrow"].play
    @sprites["uparrow"].visible = false
    @sprites["downarrow"] = AnimatedSprite.new("Graphics/UI/down_arrow", 8, 28, 40, 2, @viewport)
    # ADAPTATION: Anchor to right
    @sprites["downarrow"].x = 350 + (Graphics.width - 512) - 12
    @sprites["downarrow"].y = 260 + 16
    @sprites["downarrow"].play
    @sprites["downarrow"].visible = false
    # ADAPTATION: Anchor to right
    @sprites["markingbg"] = IconSprite.new(260 + (Graphics.width - 512), 88, @viewport)
  ...
  def drawPage(page)
    ...
    imagepos.push([ballimage, 14, @up1y - 4]) ######################## POKEBALL POS
    ...
    if status >= 0
      imagepos.push([_INTL("Graphics/UI/statuses"), 114, @down1y, 0, 20 * status, 68, 20]) ######################## STATUS POS
    end
    ...
    # Show shininess star
    imagepos.push(["Graphics/UI/shiny", 170, @up1y + 2]) if @pokemon.shiny?
    ...
    textpos = [
      [pagename, 26, 22, :left, base, shadow],  ######################## PAGE NAME POS
      [@pokemon.name, 46, @up1y, :left, Color.white, shadow],  ######################## POKEMON NAME POS
      [@pokemon.level.to_s, 46, @down1y, :left, Color.white, Color.new(192, 34, 40, 0)],  ######################## LEVEL POS
      #[_INTL("Held Item"), 66, 304, :left, Color.black, shadow]  ######################## HELD ITEM TEXT POS
    ]
	textpos.push([_INTL("Lv."), 16, @down1y, :left, Color.white, Color.new(176, 176, 176, 0)])  ################# LV. POS
    # Write the held item's name
    if @pokemon.hasItem?
      textpos.push([@pokemon.item.name, 52, 330, :left, Color.black, Color.new(176, 176, 176, 0)])   ################# ITEM NAME POS
    else
      textpos.push([_INTL("No Item"), 52, 330, :left, Color.black, Color.new(208, 216, 224, 0)])   ######### NO ITEM POS
    end
    # Write the gender symbol
    if @pokemon.male?
      textpos.push([_INTL("♂"), 88, @down1y, :left, Color.new(24, 112, 216), Color.new(136, 168, 208, 0)])  ############## MALE POS
    elsif @pokemon.female?
      textpos.push([_INTL("♀"), 88, @down1y, :left, Color.new(248, 56, 32), Color.new(224, 152, 144, 0)])  ############### FEMALE POS
    end
    # Draw all text
...
##########################################################Page 1#############################################################
  def drawPageOne
    ...
    dexNumShadow = (@pokemon.shiny?) ? Color.new(224, 152, 144) : Color.new(192, 32, 40, 0)
    
    # ADAPTATION: Define offset for readability
    ...
    if @pokemon.shadowPokemon?
      ...
      imagepos = [
        ["Graphics/UI/Summary/overlay_shadow", 224 + off_x, 240],
        ["Graphics/UI/Summary/overlay_shadowbar", 242 + off_x, 280, 0, 0, (shadowfract * 248).floor, -1]
      ]
      pbDrawImagePositions(overlay, imagepos)
    end
    # Write various bits of text
    textpos = [
      [_INTL("Dex Number"), 208 + off_x, 96, :left, base, shadow],
      [_INTL("Species Name"), 208 + off_x, 136, :left, base, shadow],
      [@pokemon.speciesName, 488 + off_x, 136, :right, Color.white, Color.new(192, 32, 40, 0)],
      [_INTL("Type"), 208 + off_x, 176, :left, base, shadow],
      [_INTL("Original Trainer"), 208 + off_x, 216, :left, base, shadow],
      [_INTL("ID Number"), 208 + off_x, 256, :left, base, shadow]
    ]
    ...
    if dexnum <= 0
      textpos.push(["???", 488 + off_x, 96, :right, Color.white, dexNumShadow])
    else
      dexnum -= 1 if dexnumshift
      textpos.push([sprintf("%03d", dexnum), 488 + off_x, 96, :right, Color.white, dexNumShadow])
    end
    # Write Original Trainer's name and ID number
    if @pokemon.owner.name.empty?
      textpos.push([_INTL("RENTAL"), 488 + off_x, 216, :right, Color.white, Color.new(176, 176, 176, 0)])
      textpos.push(["?????", 488 + off_x, 216, :right, Color.white, Color.new(176, 176, 176, 0)])
    else
      ownerbase   = Color.white
      ownershadow = Color.new(192, 32, 40, 0)
      ...
      textpos.push([@pokemon.owner.name, 488 + off_x, 216, :right, ownerbase, ownershadow])
      textpos.push([sprintf("%05d", @pokemon.owner.public_id), 488 + off_x, 256, :right,
                    Color.white, Color.new(176, 176, 176, 0)])
    end
    # Write Exp text OR heart gauge message (if a Shadow Pokémon)
    if @pokemon.shadowPokemon?
      textpos.push([_INTL("Heart Gauge"), 238 + off_x, 246, :left, base, shadow])
      ...
      drawFormattedTextEx(overlay, 234 + off_x, 308, 264, memo)
    else
      endexp = @pokemon.growth_rate.minimum_exp_for_level(@pokemon.level + 1)
      textpos.push([_INTL("Exp. Points"), 208 + off_x, 296, :left, base, shadow])
      textpos.push([@pokemon.exp.to_s_formatted, 488 + off_x, 296, :right, Color.white, Color.new(192, 32, 40, 0)])
      textpos.push([_INTL("To Next Lv."), 208 + off_x, 336, :left, base, shadow])
      textpos.push([(endexp - @pokemon.exp).to_s_formatted, 488 + off_x, 336, :right, Color.white, Color.new(192, 32, 40, 0)])
    end
    # Draw all text
    pbDrawTextPositions(overlay, textpos)
    # Draw Pokémon type(s)
    @pokemon.types.each_with_index do |type, i|
      ...
      type_x = (@pokemon.types.length == 1) ? 402 : 402 + (82 * i)
      overlay.blt(type_x + off_x - 70, 176, @typebitmap.bitmap, type_rect)
    end
    # Draw Exp bar
    if @pokemon.level < GameData::GrowthRate.max_level
      ...
      pbDrawImagePositions(overlay,
                           [["Graphics/UI/Summary/overlay_exp", 362 + off_x - 6, 372, 0, 0, w, 6]])
    end
  end
  ...
  def drawPageOneEgg
    ...
    # Show the Poké Ball containing the Pokémon
    ballimage = sprintf("Graphics/UI/Summary/icon_ball_%s", @pokemon.poke_ball)
    imagepos.push([ballimage, 14, 78 - 4])
    # Draw all images
    pbDrawImagePositions(overlay, imagepos)
    # Write various bits of text
    textpos = [
...
###########################################################Page 3######################################################
  ...
  def drawPageThree
    ...
    if @pokemon.hp > 0
      ...
      imagepos = [
        ["Graphics/UI/Summary/overlay_hp", 276 + off_x, 104, 0, hpzone * 6, w, 6]
      ]
      pbDrawImagePositions(overlay, imagepos)
    end
  end
...
##########################################################Page 4#####################################################
  ...
  def drawPageFour
    ...
    Pokemon::MAX_MOVES.times do |i|
      ...
      if move
        ...
        imagepos.push([_INTL("Graphics/UI/types"), 218 + off_x, yPos, 0, type_number * 25, 76, 25])
        textpos.push([move.name, 316 + off_x, yPos, :left, moveBase, moveShadow])
        if move.total_pp > 0
          textpos.push([_INTL("PP"), 342 + off_x, yPos + 28, :left, moveBase, moveShadow])
          ppfraction = 0
          if move.pp == 0
            ppfraction = 3
          ...
          textpos.push([sprintf("%d/%d", move.pp, move.total_pp), 460 + off_x, yPos + 28, :right, ppBase[ppfraction], ppShadow[ppfraction]])
        end
      else
        textpos.push(["-", 316 + off_x, yPos, :left, moveBase, moveShadow])
        textpos.push(["--", 442 + off_x, yPos + 32, :right, moveBase, moveShadow])
      end
      yPos += 64
    end
  ...
  def drawPageFourSelecting(move_to_learn)
    ...
    limit.times do |i|
      ...
      if move
        ...
        if move.total_pp > 0
          ...
          textpos.push([sprintf("%d/%d", move.pp, move.total_pp), 460 + off_x, yPos + 28, :right,
    ...
    @pokemon.types.each_with_index do |type, i|
      type_number = GameData::Type.get(type).icon_position
      type_rect = Rect.new(0, type_number * 25, 76, 25)
      type_x = (@pokemon.types.length == 1) ? 130 : 96 + (82 * i)
  ...
  def drawSelectedMove(move_to_learn, selected_move)
    ...
    if selected_move.display_accuracy(@pokemon) == 0
      textpos.push(["---", 216, 192, :right, Color.white, shadow])
    else
  ...
  def drawPageFive
    ...
    (@ribbonOffset * 4...(@ribbonOffset * 4) + 12).each do |i|
      ...
      imagepos.push(["Graphics/UI/Summary/ribbons",
                     230 + (68 * (coord % 4)) + off_x - 12, 78 + 16 + (68 * (coord / 4).floor),
    ...
    loop do
      ...
      if redraw
        ...
        textpos = [
          ...
          [_INTL("Ok"), 366 + off_x, 254 - 4, :center, Color.black, shadow],
          [_INTL("Cancel"), 366 + off_x, 304 - 4, :center, Color.black, shadow]
        ]
