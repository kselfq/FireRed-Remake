class Game_Character
  ...
  def move_speed=(val)
    ...
    #   6 => 0.05
    case val
    when 6 then @move_time = 0.0333  # 0.05 * (2/3)
    when 5 then @move_time = 0.0666  # 0.1 * (2/3)
    else        @move_time = (4.0 / 3.0) / (2**val) # Standard formula * (2/3)
    end
  end
  ...
  def jump_speed=(val)
    @jump_speed = val
    case val
    when 6 then @jump_time = 0.0333
    when 5 then @jump_time = 0.0666
    else        @jump_time = (4.0 / 3.0) / (2**val)
    end
  end
