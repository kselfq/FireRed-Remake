...
EliteBattle.defineMoveAnimation(:SHADOWBALL) do
  ...
  fp["bg"].bitmap.fill_rect(0,0,fp["bg"].bitmap.width,fp["bg"].bitmap.height,Color.new(76,0,154))
  fp["bg"].opacity = 0
  #
  for j in 0...16
  ...
  for i in 0...64
    ...
    fp["circle"].angle += 8
	pbSEPlay("EBDX/Anim/ghost3", 50) if i%8==0
    for j in 0...16
      next if j > i/4
      if fp["p#{j}"].ox >= -16
        fp["p#{j}"].ox = -128
