...
#  Shadow Chill
#-------------------------------------------------------------------------------
EliteBattle.defineMoveAnimation(:SHADOWCHILL) do
  ...
  for i in 0...16
    ...
    fp["c#{i}"].bitmap = pbBitmap("Graphics/EBDX/Animations/Moves/eb250_2")
  ...
  for i in 0...72
    ...
    fp["#{i}"].bitmap = pbBitmap("Graphics/EBDX/Animations/Moves/eb243_1")
  ...
  for i in 0...96
    ...
    if i >= 32
      @targetSprite.tone.red += Settings::SHADOW_MOVES_RED if @targetSprite.tone.red < Settings::SHADOW_MOVES_RED_LIM
      @targetSprite.tone.green += Settings::SHADOW_MOVES_GREEN if @targetSprite.tone.green < Settings::SHADOW_MOVES_GREEN_LIM
      @targetSprite.tone.blue += Settings::SHADOW_MOVES_BLUE if @targetSprite.tone.blue < Settings::SHADOW_MOVES_BLUE_LIM
      @targetSprite.still
    end
  ...
  20.times do
    @targetSprite.tone.red -= Settings::SHADOW_MOVES_RED
    @targetSprite.tone.green -= Settings::SHADOW_MOVES_GREEN
    @targetSprite.tone.blue -= Settings::SHADOW_MOVES_BLUE
    @targetSprite.still
    fp["bg"].opacity -= 15
    @scene.wait(1,true)
  end
