...
class Battle::Scene
  ...
  #-----------------------------------------------------------------------------
    ...
    if args[0].is_a?(Array)
      ...
      data = EliteBattle.get_vector(args[0], @battle)
      x, y, angle, scale, zoom = data
    else
      return if args.length < 5
      x, y, angle, scale, zoom = args
    end
    
    # FORCED OFFSET IN MULTI-BATTLES
    is_multi = @battle && (@battle.doublebattle? || @battle.triplebattle?)
    if is_multi && (args[0] == :SENDOUT || args[0] == :PLAYER || args[0] == :DUAL)
      x += 1400
      y += 1600
    end
