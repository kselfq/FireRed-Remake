...
class DataBoxEBDX  <  SpriteWrapper
 ...
 #-----------------------------------------------------------------------------
  ...
  def initialize(battler, viewport = nil, player = nil, scene = nil)
	...
	@triplebattle = @battle.triplebattle?
    ...
    # --- ADDED: Variables for Status Animation States ---
    # 0: Fading Out, 1: Paused (Dim), 2: Fading In, 3: Paused (Visible)
    @status_anim_state = 0 
    @status_anim_timer = 0
  end
  #-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
def appear
  @inposition = true        # mark as in position immediately
  @loaded = true
  self.x = self.defX        # immediately set final X
  self.y = self.defY        # immediately set final Y
end
...
#-----------------------------------------------------------------------------
  ...
  #-----------------------------------------------------------------------------
  def defX
    # Default calculation for Single Battles
    x = @defX
    x += (@battler.index/2)*8 if @playerpoke
    x += (@battle.pbParty(1).length - 1 - @battler.index/2)*8 - (@battle.pbParty(1).length - 1)*8 if !@playerpoke

    # --- TRIPLE BATTLE OVERRIDE ---
    # We check battle size OR the variable to be safe
    if @triplebattle || @battle.pbMaxSize(0) == 3
      case @battler.index
      # PLAYER SIDE
      when 0 # Player Left
        return 8
      when 2 # Player Center
        return 260
      when 4 # Player Right
        return 512
        
      # OPPONENT SIDE (Explicitly separated)
      when 1 # Opponent Right (Visually Left)
        return Graphics.width - 756
      when 3 # Opponent Center
        return Graphics.width - 504
      when 5 # Opponent Left (Visually Right)
        return Graphics.width - 252
      end
    end

    # --- DOUBLE BATTLE OVERRIDE ---
    if @doublebattle || @battle.pbMaxSize(0) == 2
      case @battler.index
      when 0 # Player Left
        return 8
      when 2 # Player Right
      ...
      when 1 # Opponent Right (Visually Left)
        return Graphics.width - 504
      when 3 # Opponent Left (Visually Right)
        return Graphics.width - 252
      end
    end
  ...
  def defY
    ...
    # --- TRIPLE BATTLE OVERRIDE ---
    if @triplebattle || @battle.pbMaxSize(0) == 3
      # Player side is bottom (646), Opponent side is top (24)
      return 646 if @playerpoke  # Indices 0, 2, 4
      return 24  if !@playerpoke # Indices 1, 3, 5
    end

    # --- DOUBLE BATTLE OVERRIDE ---
    if @doublebattle || @battle.pbMaxSize(0) == 2
      # Player side is bottom (646), Opponent side is top (24)
      return 646 if @playerpoke  # Indices 0, 2
      return 24  if !@playerpoke # Indices 1, 3
    end
  ...
  def setUp
    ...
    # --- CHANGED: Initialize opacity to 0 for fade-in effect ---
    @sprites["status"].opacity = 0 
    # -----------------------------------------------------------
    ...
    @sprites["container"].src_rect.height = @showexp ? 26 : 9
    ...
    @sprites["hp"].bitmap = Bitmap.new(1, 4)
    ...
    @sprites["caught"].ex = @sprites["container"].ex - 14
    @sprites["caught"].ey = @sprites["container"].ey - 3
  ...
  def updateHpBar
    ...
    @sprites["textHP"].bitmap.font.size = 18
    
    if @showhp
      # Calculate X to center the text manually (since we are switching to OutlineText)
      # Original Anchor: @sprites["textHP"].bitmap.width - @hpTextX - 162
      #anchor_x = @sprites["textHP"].bitmap.width - @hpTextX - 182
      text_width = @sprites["textHP"].bitmap.text_size(str).width
      #draw_x = anchor_x - (text_width / 2)
      
      # Draw Outlined Text (Color: White, Outline: Dark Green)
      pbDrawOutlineText(@sprites["textHP"].bitmap, 18, @hpTextY - 11, text_width + 10, 32, str, Color.new(255, 255, 255), Color.new(4,91,75), 2)
    end
  end
  ...
  def refresh
    ...
    @sprites["textName"].bitmap.font.size = 24
    ...
    str += " "
    color = @pokemon.shiny? ? Color.new(255, 255, 255) : Color.new(255, 255, 255)
    pbDrawOutlineText(@sprites["textName"].bitmap,18-o,3,@sprites["textName"].bitmap.width-40,@sprites["textName"].bitmap.height,str,color,Color.new(0,0,0,0),0)
    ...
    pbDrawOutlineText(@sprites["textName"].bitmap,226,3,@sprites["textName"].bitmap.width,@sprites["textName"].bitmap.height,str,color,Color.new(0,0,0,0),0)
    
    # writes the Pokemon's level
    @sprites["textName"].bitmap.font.size = 20
    str_level = "Lv.#{@battler.level}"
    box_width = @sprites["textName"].bitmap.width - 34  
    # Right Aligned (2) with Outline
    pbDrawOutlineText(@sprites["textName"].bitmap, 0, 7, box_width, 32, str_level, Color.new(255, 255, 255), Color.new(4,91,75), 2)
  ...
  def update
    ...
    # --- CHANGED: Breathing Animation (0 to 255 Opacity) ---
    status = EliteBattle.GetStatusIconPosition(@battler.status)
    if status > 0
      # Ensure correct icon
      ...
      case @status_anim_state
      when 0 # Fading Out
        @sprites["status"].opacity -= 4
        # Lower limit changed to 0 (Invisible)
        if @sprites["status"].opacity <= 0
          @sprites["status"].opacity = 0
          @status_anim_state = 1
          @status_anim_timer = 0
        end
        
      when 1 # Paused at Min (Invisible)
        @status_anim_timer += 1
        # Wait approx 1 second (40 frames)
        if @status_anim_timer >= 40
          @status_anim_state = 2
          @status_anim_timer = 0
        end
      ...
      when 2 # Fading In
        @sprites["status"].opacity += 4
        if @sprites["status"].opacity >= 255
          @sprites["status"].opacity = 255
          @status_anim_state = 3
          @status_anim_timer = 0
        end
        
      when 3 # Paused at Max (Visible)
        @status_anim_timer += 1
        # Wait approx 1 second (40 frames)
        if @status_anim_timer >= 40
          @status_anim_state = 0
...
class SafariDataBoxEBDX < SpriteWrapper
  ...
  def refresh
    ...
    pbDrawOutlineText(self.bitmap,0,38,self.bitmap.width,self.bitmap.height,str,Color.new(255, 255, 255),Color.new(0,0,0,0),1)
  end
  #-----------------------------------------------------------------------------
  #  update (temp)
  #-----------------------------------------------------------------------------
